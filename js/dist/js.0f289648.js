...
if (!previewEl || !metaEl) throw new Error("Required DOM nodes not found.");
let currentSvg = "";
...
function loadStateFromStorage() {
    try {
        ...
        const fullSphere = mode === "single" || mode === "side-by-side" || mode === "flower" ? mode : defaults.fullSphere;
        return {
...
function saveStateToStorage(cfg) {
    try {
...
function setControllerHint(controller, hint) {
    controller.domElement.title = hint;
    const textNodes = controller.domElement.querySelectorAll(".property-name, input, select, button");
    for (const node of textNodes)if (node instanceof HTMLElement) node.title = hint;
}
function sanitizeToken(text) {
    const cleaned = text.replace(/[^a-zA-Z0-9]+/g, "-").replace(/^-+|-+$/g, "").toLowerCase();
    return cleaned || "x";
}
function formatValueForFilename(value) {
    if (typeof value === "number") return sanitizeToken(`${value}`.replace(/-/g, "m").replace(/\./g, "p"));
    return sanitizeToken(value);
}
function autoFilename(cfg) {
    const parts = [
        "lune"
    ];
    const entries = [
...
function maxCosOnInterval(a, b) {
    const lo = Math.min(a, b);
    const hi = Math.max(a, b);
    const twoPi = 2 * PI;
    const k = Math.ceil(lo / twoPi);
    if (k * twoPi <= hi) return 1;
    return Math.max(Math.cos(lo), Math.cos(hi));
}
function goreBounds(radius, gores, latMin, latMax) {
    if (gores < 1) throw new Error("gores must be >= 1");
    if (radius <= 0) throw new Error("diameter must be > 0");
    if (latMin >= latMax) throw new Error("lat range invalid: lat_min must be < lat_max");
...
function halfWidthAtLat(radius, deltaLambda, lat) {
    const halfWidth = 0.5 * radius * deltaLambda * Math.cos(lat);
    return halfWidth > 0 ? halfWidth : 0;
}
function scaleFactorsFromDeltas(baseWidth, baseHeight, scaleXMm, scaleYMm) {
    const targetWidth = baseWidth + scaleXMm;
    const targetHeight = baseHeight + scaleYMm;
    if (targetWidth <= 0) throw new Error("--scale-x-mm makes peel width <= 0");
    if (targetHeight <= 0) throw new Error("--scale-y-mm makes peel height <= 0");
    return [
        targetWidth / baseWidth,
        targetHeight / baseHeight
    ];
}
function transformPoint(x, y, cx, cy, sx, sy, tx, ty) {
    return [
        cx + (x - cx) * sx + tx,
        cy + (y - cy) * sy + ty
    ];
}
function rotatePoint(x, y, pivotX, pivotY, angleRad) {
...
function gorePolygonPoints(radius, gores, latMin, latMax, segments, cx, cy, sx, sy) {
    if (segments < 1) throw new Error("samples must be >= 1");
    const deltaLambda = 2 * PI / gores;
    ...
    if (xTop > 1e-12) points.push(transformPoint(xTop, yTop, cx, cy, sx, sy, 0, 0));
    ...
    if (xBottom > 1e-12) points.push(transformPoint(-xBottom, yBottom, cx, cy, sx, sy, 0, 0));
    for(let i = 0; i < segments; i += 1){
...
function polygonToPath(points) {
    if (points.length === 0) throw new Error("polygon must contain at least one point");
    const cmds = [
        `M ${points[0][0].toFixed(6)} ${points[0][1].toFixed(6)}`
    ];
    for(let i = 1; i < points.length; i += 1)cmds.push(`L ${points[i][0].toFixed(6)} ${points[i][1].toFixed(6)}`);
    cmds.push("Z");
    return cmds.join(" ");
}
function buildSvg(cfg) {
    ...
    const polygons = [];
    if (cfg.fullSphere === "side-by-side") for(let i = 0; i < gores; i += 1){
        const ox = i * baseWidth;
        polygons.push(basePoints.map(([x, y])=>[
                x + ox,
                y
            ]));
    }
    ...
    const paths = polygons.map((poly)=>poly.map(([x, y])=>[
                x - globalMinX,
                y - globalMinY
            ])).map((poly)=>polygonToPath(poly)).map((d)=>`<path d="${d}" fill="${DEFAULT_FILL}" fill-opacity="${DEFAULT_FILL_OPACITY.toFixed(3)}" ` + `stroke="${DEFAULT_STROKE}" stroke-width="${DEFAULT_STROKE_WIDTH_MM.toFixed(6)}" />`).join("\n");
    const svg = `<?xml version="1.0" encoding="UTF-8"?>\n` + `<svg xmlns="http://www.w3.org/2000/svg" width="${width.toFixed(6)}mm" height="${height.toFixed(6)}mm" viewBox="0 0 ${width.toFixed(6)} ${height.toFixed(6)}">\n` + `${paths}\n` + `</svg>\n`;
    return {
        svg,
        width,
        height,
...
function render() {
    try {
        ...
        metaEl.textContent = `Canvas: ${result.width.toFixed(3)} x ${result.height.toFixed(3)} mm | Pieces: ${result.layoutCount}`;
    } catch (error) {
        previewEl.innerHTML = "";
...
const gui = new _datGui.GUI({
    name: "Lune Gore Controls",
    width: 340
});
const diameterController = gui.add(state, "diameter", 1, 2000, 0.1).name("diameter (mm)").onChange(render);
setControllerHint(diameterController, "Sphere diameter in millimeters. This sets the physical size of each generated gore in the exported SVG.");
const goresController = gui.add(state, "gores", 1, 64, 1).name("gores").onChange(render);
setControllerHint(goresController, "Number of gores needed to cover the sphere. Higher values produce narrower individual gores.");
const fullSphereController = gui.add(state, "fullSphere", {
    single: "single",
    "side-by-side": "side-by-side",
    flower: "flower"
}).name("full-sphere").onChange(render);
setControllerHint(fullSphereController, "Layout mode: single = one gore, side-by-side = all gores in a strip, flower = radial petals touching at one point.");
const scaleXController = gui.add(state, "scaleXMm", -50, 50, 0.1).name("scale-x-mm").onChange(render);
setControllerHint(scaleXController, "Adds width per gore in millimeters after projection. Use small values for fit compensation.");
const scaleYController = gui.add(state, "scaleYMm", -50, 50, 0.1).name("scale-y-mm").onChange(render);
setControllerHint(scaleYController, "Adds height per gore in millimeters. In flower mode, petals are shifted so the outer reference circle remains stable.");
const latMaxController = gui.add(state, "latMax", 0, 90, 1).name("lat-max").onChange(render);
setControllerHint(latMaxController, "Upper latitude limit in degrees. Effective range is from -90 to +lat-max.");
const samplesController = gui.add(state, "samples", 1, 240, 1).name("samples").onChange(render);
setControllerHint(samplesController, "Curve resolution (segments per edge). Higher values make smoother outlines but larger SVG paths.");
const actions = {
    downloadSvg: ()=>{
        if (currentSvg.length > 0) triggerDownload(currentFilename, currentSvg);
    }
};
const downloadController = gui.add(actions, "downloadSvg").name("Download SVG");
setControllerHint(downloadController, "Downloads the currently displayed SVG using current parameters and physical mm dimensions.");
render();

},{"dat.gui":"329hF"}],"329hF":[function(require,module,exports,__globalThis) {
/**
...
},{}]},["elbaT","gNc1f"], "gNc1f", "parcelRequire9fb2", {})

//# sourceMappingURL=js.0f289648.js.map
